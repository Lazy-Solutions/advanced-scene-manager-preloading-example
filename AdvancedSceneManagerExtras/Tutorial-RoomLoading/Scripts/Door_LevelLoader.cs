...
public class Door_LevelLoader : MonoBehaviour
{
    ...
    public void OnPreloadTriggerEnter()
    {
        ...
        if (!isOpen && !isOpen.isPreloaded)
            StartCoroutine(Preloader.Preload(SceneToLoad));
    }
    ...
    void OpenDoor()
    {
        ...
        StartCoroutine(TweenOpenDoor(startPos, endPos, () =>
            StartCoroutine(countdown(stayOpenFor, () =>
                StartCoroutine(TweenOpenDoor(endPos, startPos, () => isAnimating = false))
            ))
        ));
    }
